    ...
    <div className="space-y-4">
      <div className="flex flex-col items-center gap-3 py-4">
        <div className="relative">
          <img src={user.avatar_url} alt={user.username} className="w-20 h-20 rounded-full object-cover ring-4 ring-gray-100" />
    ...
    <div className="space-y-3">
      {items.map(({ key, label }) => {
        ...
        <div key={key} className="flex items-center justify-between py-2">
          ...
          <button
            onClick={() => setSettings((s) => ({ ...s, [settingKey]: !s[settingKey] }))}
            className={`relative w-11 h-6 rounded-full transition-colors ${settings[settingKey] ? 'bg-blue-600' : 'bg-gray-200'}`}
          >
            <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${settings[settingKey] ? 'translate-x-5' : 'translate-x-0.5'}`} />
          </button>
        </div>
        );
      })}
