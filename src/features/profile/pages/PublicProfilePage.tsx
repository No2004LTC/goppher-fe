...
export default function PublicProfilePage() {
  ...
  const handleFollowToggle = async () => {
    ...
    setFollowing(!prevFollowing); // Optimistic UI
    ...
    return (
      <MainLayout>
        <div className="text-center py-20 text-gray-500 font-medium">
          ...
          <button
            onClick={() => navigate(-1)}
            className="mt-6 px-5 py-2 bg-blue-600 text-white rounded-xl font-semibold text-sm hover:bg-blue-700 transition"
          >
            Quay lại
          </button>
        </div>
  ...
  const postsCount = profileUser.posts_count ?? userPosts.length;
  ...
  return (
    <MainLayout>
      ...
      <div className="bg-white rounded-3xl border border-gray-100 overflow-hidden mb-6 shadow-sm">
        ...
        {/* Ảnh bìa */}
        <div
          className="h-48 bg-gradient-to-r from-blue-500 to-sky-300 relative"
          style={
        ...
        <div className="px-8 relative pb-8">
          ...
          <div className="flex justify-end pt-4 mb-8 gap-3">
            {isMyProfile ? (
              ...
              <>
                <button
                  onClick={() => profileUser?.id && navigate(`/chat?to_user=${profileUser.id}`)}
                  className="px-6 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold rounded-xl flex items-center gap-2 transition-all shadow-sm"
                >
                ...
                <button
                  ...
                  className={`px-6 py-2.5 font-bold rounded-xl flex items-center gap-2 transition-all shadow-md disabled:opacity-60 ${following
          ...
          <div className="mt-2">
            ...
            <p className="mt-5 text-gray-800 leading-relaxed max-w-2xl text-[15px]">
              {profileUser.bio || (
                isMyProfile
                  ? 'Bạn chưa có tiểu sử. Hãy vào phần Cài đặt để thêm vài dòng giới thiệu!'
                  : `${profileUser.username} chưa có tiểu sử.`
              )}
            </p>
            ...
            <div className="flex items-center gap-8 mt-6 pt-6 border-t border-gray-50">
              <div className="text-center cursor-pointer hover:opacity-80 transition">
                <span className="block font-black text-xl text-gray-900">{postsCount}</span>
      ...
      <div className="flex gap-6">
        <div className="w-full lg:w-2/3 space-y-4">
          <div className="bg-white p-5 rounded-2xl border border-gray-100 font-black text-gray-900 text-lg shadow-sm">
            Bài viết của {profileUser.username}
          </div>
          ...
          {userPosts.length > 0 ? (
            ...
            <div className="bg-white py-16 rounded-2xl border border-gray-100 text-center shadow-sm">
              <p className="text-gray-500 font-medium">
                {isMyProfile ? 'Bạn chưa có bài viết nào.' : `${profileUser.username} chưa có bài viết nào.`}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="hidden lg:block w-1/3">
          <div className="bg-white p-6 rounded-2xl border border-gray-100 sticky top-24 shadow-sm">
            ...
            <p className="text-sm text-gray-600 leading-relaxed">
              {profileUser.bio
                ? profileUser.bio
                : `Chào mừng đến với không gian cá nhân của `}
              {!profileUser.bio && (
                <span className="font-bold">{profileUser.username}</span>
              )}
              {!profileUser.bio && '. Nơi chia sẻ những khoảnh khắc và kết nối với mọi người.'}
            </p>
            {!isMyProfile && (
              <button
                onClick={handleFollowToggle}
                disabled={followLoading}
                className={`w-full mt-4 py-2 font-bold rounded-xl text-sm transition ${following
