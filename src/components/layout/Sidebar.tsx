...
import { Home, MessageCircle, Bell, Settings, LogOut, Users, Bookmark, Zap } from 'lucide-react';
...
export default function Sidebar() {
  // 👉 ĐÃ XÓA setIsNotifPanelOpen vì hệ thống không còn dùng Panel nữa
  const { user, logout, unreadChatCount, unreadNotifCount } = useApp();
  ...
  return (
    <aside className="hidden lg:flex flex-col w-64 h-screen bg-white border-r border-gray-100 fixed left-0 top-0 z-20">
      ...
      <div className="px-4 py-4 border-b border-gray-100">
        <NavLink
          to={`/profile/${user?.username}`} // Đổi thành route có username cho chuẩn với API của cậu
          className={({ isActive }) =>
            `flex items-center gap-3 p-2 rounded-xl cursor-pointer transition-colors group ${isActive ? 'bg-blue-50 ring-1 ring-blue-100' : 'hover:bg-gray-50'
            }`
          }
        >
          {({ isActive }) => (
      ...
      <nav className="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
        {navItems.map(({ icon: Icon, label, to }) => (
          <NavLink
            key={to}
            to={to}
            // 👉 LỖI Ở ĐÂY ĐÃ BỊ LOẠI BỎ: Không còn onClick chặn trang nữa
            className={({ isActive }) =>
              `w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${isActive
            ...
            {({ isActive }) => {
              ...
              return (
                <>
                  ...
                  {badgeCount > 0 && (
                    <span
                      className={`ml-auto text-[10px] font-bold px-1.5 min-w-[20px] h-5 rounded-full flex items-center justify-center shadow-sm ${isActive ? 'bg-white text-blue-600' : 'bg-red-500 text-white'
                        }`}
                    >
